...
public class AdsFeature implements GadgetFeature {
   ...
   * 
   * <p>In order to enable clickthrough tracking on this link, you must use
   * this method to initiate the redirect. When this method is called, a
   * clickthrough hit is received and then redirected to the destination.
   * 
   ...
   * 
   * <p>Use this when your gadget contains only a single user-interaction that
   * needs to be tracked. For example, this could be a click-to-play button
   * which starts video playback or a start button to load in a Flash game.
   * Each time this function is called, a generic hit is sent to the tracking
   * server. These hits are grouped into "Primary Interactions" in the
   * interaction reports.
   */
   ...
   * 
   * <p>Use this function when your gadget needs to track multiple user
   * interaction types. 
   * 
   ...
   * 
   * <p>Use this function when you want pass in a value for a specific
   * interaction type to be summed
   * up in the reports.
   * 
   * @param name The name of the reported interaction. It must match one from
   * the supported list of interactions
   ...
   * the destination URL.
   * 
   * <p>This function is a combination of two methods:
   ...
   * @param value value to sum up for the reported interaction. It must be a
   * positive number This parameter is optional, {@code null} is accepted
   */
  public native void reportInteractionClick(String destUrl, String name, String value) /*-{
