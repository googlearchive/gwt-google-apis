...
/**
 * Determines, which sub-class of {@link ContentSection} to instantiate, based
 * on the current gadget view.
 */
public class ContentSectionGenerator extends Generator {
  ...
  public String generate(TreeLogger logger, GeneratorContext context, String typeName)
    ...
    try {
      viewsProperty = propertyOracle.getPropertyValue(logger, GADGET_VIEW_PROPERTY);
      views = viewsProperty.split(",");
    } catch (BadPropertyValueException e) {
      throw new UnableToCompleteException();
    }
    JClassType gadgetClassType = findGadgetClassType(logger, typeOracle, propertyOracle);
    // These are the view types the user specified with the Content annotation
    GadgetViewType[] gadgetViewTypes =
        GadgetUtils.getViewTypes(logger, gadgetClassType, typeOracle);
   ...
   *         found.
   */
  private static JClassType findGadgetClassType(TreeLogger logger, TypeOracle typeOracle,
    ...
    try {
      ...
      String gadgetMainClass = propertyOracle.getPropertyValue(logger, MAINCLASS_PROPERTY);
      if (gadgetMainClass.length() > 0) {
        ...
        logger.log(TreeLogger.WARN, "'" + MAINCLASS_PROPERTY + "' property found, but '"
            + gadgetMainClass + "' is not a Gadget sub-class.");
      }
    } catch (BadPropertyValueException e) {
      // Just fall-through.
    }
    ...
    if (gadgetSubTypes.length == 1) {
      ...
      logger.log(TreeLogger.WARN, "More than one Gadget subclass has been found. Choosing: "
