...
public class Transliteration {
  ...
  public static final void transliterate(ArrayList<String> wordsArray,
      LanguageCode srcLang, LanguageCode destLang, TransliterationCallback callback) {
    JsArrayString jsArrayStr = (JsArrayString) JavaScriptObject.createArray();
    for (int i = 0; i < wordsArray.size(); ++i) {
      jsArrayStr.set(i, wordsArray.get(i));
    }
    transliterate(jsArrayStr, srcLang.getLangCode(), destLang.getLangCode(), callback);
  }
   ...
   * @param callback the callback that receives the result
   */
  private static final native void transliterate(JsArrayString wordsArray,
